                    ...
                    for reference in references:
                        for method in methods:
                            for level in levels:
                                ...
                                out_file = results_dir / f"scib_metrics_panel_singlets/raw/{name}/scib_metrics_{layer}_{reference}_{method}_{level}_{n_comps=}_{max_n_cells=}.parquet"
                                out_files_panel.append(out_file)

                                rule:
                                    name: f'scib_metrics_panel_singlets/raw/{name}/scib_metrics_{layer}_{reference}_{method}_{level}'
                                    input:
                                        panel=panel,
                                    output:
                                        out_file=out_file,
                                    params:
                                        ...
                                        singlets="--singlets" if singlets else "",
                                    threads: 1
                                    resources:
                                        mem='80GB',
                                        runtime='5h',
...
rule scib_metrics_panel_singlets_all:
    input:
        out_files_panel
    output:
        touch(results_dir / "scib_metrics_singlets.done")