...
for segmentation in (segmentations := cell_type_annotation_dir.iterdir()):
    if segmentation.stem == 'proseg_v1':
        continue
    for condition in (conditions := segmentation.iterdir()): 
        for panel in (panels := condition.iterdir()):
            for preprocessing in (preprocessings := panel.iterdir()): 
                for reference in (references_dir := preprocessing.iterdir()):
                    for method in (methods_dir := reference.iterdir()):
