...
for genes_name, genes in genes_dict.items():
    for reference in (references := scrnaseq_processed_data_dir.iterdir()):
        ...
        out_file = results_dir / f'embed_panel_restricted_genes_scrnaseq/{genes_name}/{reference_name}/umap_{layer}_{n_comps=}_{n_neighbors=}_{min_dist=}_{metric}.parquet' 
        out_files.append(out_file)

        rule:
            name: f'embed_panel_restricted_genes_scrnaseq/{genes_name}/{reference_name}'
            input:
