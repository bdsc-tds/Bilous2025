...
def QC(sdata, sample_name = '', save_path=None):
    ...
    perc_transcripts_decoded_q20 = metrics_summary['fraction_transcripts_decoded_q20']*100
    estimated_number_of_false_positive_transcripts_per_cell = metrics_summary['estimated_number_of_false_positive_transcripts_per_cell']

    # extra QC metrics on HQ genes
    adata.obs['n_genes_by_counts'] = (adata.X>0).sum(1).A1
    hq_gene_transcripts = sdata['transcripts'][sdata['transcripts']['feature_name'].isin(adata.var_names) & (sdata['transcripts']['qv']>20)]
    total_hq_gene_transcripts = len(hq_gene_transcripts.index)
    total_hq_transcripts_by_gene = hq_gene_transcripts['feature_name'].value_counts().compute()

    df_unassigned_hq_gene_transcripts = hq_gene_transcripts[hq_gene_transcripts["cell_id"] == "UNASSIGNED"]

    unassigned_hq_gene_transcripts = len(df_unassigned_hq_gene_transcripts.index)
    unassigned_hq_transcripts_by_gene = df_unassigned_hq_gene_transcripts["feature_name"].value_counts().compute()
    perc_unassigned_hq_transcripts = unassigned_hq_gene_transcripts / total_hq_gene_transcripts * 100
    perc_unassigned_hq_transcripts_by_gene = unassigned_hq_transcripts_by_gene / total_hq_transcripts_by_gene * 100
    ...
    table_ax = fig.add_axes([0.32, 0.8, 0.4, .8])  # x, y, width, height
    table_ax.axis('off')
    ...
    for i, (metric, value) in enumerate(zip(title_content["QC metrics"], title_content[""])):
        table.add_cell(i, 0, col_widths[0], 1 / len(title_content["QC metrics"]), text=metric, loc='left')
        table.add_cell(i, 1, col_widths[1], 1 / len(title_content["QC metrics"]), text=value, loc='center')

    # Add column headers
    table.add_cell(-1, 0, col_widths[0], 1 / len(title_content["QC metrics"]), text=sample_name, loc='center', facecolor='lightblue')
    table.add_cell(-1, 1, col_widths[1], 1 / len(title_content["QC metrics"]), text="", loc='center', facecolor='lightblue')
