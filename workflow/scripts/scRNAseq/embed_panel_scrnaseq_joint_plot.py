...
parser.add_argument("--reference1", type=Path, help="annotation reference1")
parser.add_argument("--reference2", type=Path, help="annotation reference2")
...
df["cell_id"] = df.index
annot1 = pd.read_parquet(reference1 / "metadata.parquet").set_index("cell_id")[color]
annot2 = pd.read_parquet(reference2 / "metadata.parquet").set_index("cell_id")[color]
